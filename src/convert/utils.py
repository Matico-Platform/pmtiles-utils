...
def check_compatibility() -> bool:
    ...
    try:
        gdal_output = subprocess.run(
            ["ogrinfo", "--formats"], check=True, capture_output=True
        )
    ...
    if gdal_is_valid:
        ...
        click.secho(
            "Error: the available GDAL / ogr2ogr does not support FlatGeoBuf. Please install version 3.1 or newer.",
            bg="red",
            bold=True,
        )
        is_valid = False
    ...
    tc_v = subprocess.run(
        ["tippecanoe -v"], shell=True, text=True, capture_output=True
    )
    ...
    if tc_is_valid:
        ...
        click.secho(
            "Error: Tippecanoe is not installed or the available Tippecanoe not support FlatGeoBuf. Please install version 2.1 or higher. See https://github.com/felt/tippecanoe.",
            fg="red",
            underline=True,
            bold=True,
        )
